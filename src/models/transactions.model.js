...
const createDetailTransaction = (client, body, transactionId) => {
  return new Promise((resolve, reject) => {
    const { products } = body;
    let sql =
      "INSERT INTO transactions_products_sizes (transaction_id, product_id, size_id, qty, subtotal) VALUES ";
    let values = [];
...
const getTransaction = (client, transactionId) => {
  return new Promise((resolve, reject) => {
    const sql = `select u.email, u.address, p."name" as "product", s."name" as "size", pr.code as "promo", py."method" as "payment_method", 
